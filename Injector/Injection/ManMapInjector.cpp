...
/* Modified version of https://github.com/TheCruZ/Simple-Manual-Map-Injector/blob/master/Manual%20Map%20Injector/injector.cpp */

#ifdef _WIN64
#define IMAGE_ARCH IMAGE_FILE_MACHINE_AMD64
#else
#define IMAGE_ARCH IMAGE_FILE_MACHINE_I386
#endif
...
//The shellcode needs to be specific, so we should keep optimizations off for this TU
#pragma optimize( "", off )
...
bool ManualMap(HANDLE hProc, std::vector<uint8_t> dllData, bool ClearHeader, bool ClearNonNeededSections, bool AdjustProtections, bool SEHExceptionSupport, DWORD fdwReason, LPVOID lpReserved) {
	...
	if (pOldFileHeader->Machine != IMAGE_ARCH) {
	...
	//Correct the shellcode pointer if its wrong
	char* shellcodeBytes = (char*)Shellcode;
	while (shellcodeBytes[0] != 0x48) {
		int jmpOffset = *(int*)&shellcodeBytes[1];
		uintptr_t shellcodeAddr = (uintptr_t)shellcodeBytes;
		shellcodeBytes = (char*)(shellcodeAddr + jmpOffset + 5); //idk why 5 but yeah... maybe bc 5 is the size of the instruction? not sure
	}

	if (!WriteProcessMemory(hProc, pShellcode, shellcodeBytes, 0x1000, nullptr)) {
